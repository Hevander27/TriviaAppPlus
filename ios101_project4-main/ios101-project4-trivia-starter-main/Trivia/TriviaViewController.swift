...
class TriviaViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    private func fetchTriviaQuestions() throws {
        var urlString = "https://opentdb.com/api.php?amount=10&type=boolean"
        // lines 35 -37 executed
        guard let url = URL(string: urlString) else {
            throw URLError(.badURL)
        }
        // Use the apiUrl if available, else use the default URL
        if urlString == "https://opentdb.com/api.php?amount=10&type=boolean" {
